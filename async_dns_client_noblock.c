...
#define DNS_HOST			0x01				// IPv4地址
#define DNS_CNAME			0x05				// 规范名
...
将域名分成了三段，分别是 www，google，com
每一段开头都会有一个字节，表示后面跟着的那段域名的字节长度
*/
static void dns_parse_name(unsigned char *chunk, unsigned char *ptr, char *out, int *len) {
	...
	while (1) {
		...
		if (is_pointer(flag)) { // 读到域名前的字节
			
			n = (int)ptr[1]; // 获取域名的长度，如 \3 n就为3
			ptr = chunk + n; // 后移，处理下一段
...
static int dns_parse_response(char *buffer, struct dns_item **domains) {
	...
	for (i = 0;i < answers;i ++) {
		...
		} else if (type == DNS_HOST) { // IPv4地址

			bzero(ip, sizeof(ip));

			if (datalen == 4) { // 4个字节，1个字节8bit, 二进制表示 0~255
				memcpy(netip, ptr, datalen);
				inet_ntop(AF_INET , netip , ip , sizeof(struct sockaddr)); // 化为点分十进制
...
//epoll_wait
//result callback
static void* dns_async_client_proc(void *arg) { // 线程执行函数
	...
	while (1) {
		...
		for (i = 0;i < nready;i ++) {
			...
			int count = dns_parse_response(buffer, &domain_list); // 解析出多少个 ip 地址

			data->cb(domain_list, count); // 调用回调函数
...
//epoll init
//thread init
struct async_context *dns_async_client_init(void) { // 初始化

	int epfd = epoll_create(1); // 初始化 epoll
	...
	pthread_t thread_id; 
	int ret = pthread_create(&thread_id, NULL, dns_async_client_proc, ctx); // 初始化线程
	if (ret) {
		perror("pthread_create");
		return NULL;
	}
	usleep(1); // 延迟 1 微秒, child go first
